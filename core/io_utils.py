...
from models.constants import Performance, Validation, PathsIni, Visualization, FileTypes, PlaterFormat, FigureProperties
...
def _set_pdf_metadata(pdf: PdfPages) -> None:
    """Write standard MPLACE metadata into a PdfPages document."""
    d = pdf.infodict()
...
def convert_to_pharmbio_format(layout_text_array: List[str]) -> List[str]:
    ...
    if len(header) == 6 and [x.strip() for x in header] == ['plateID', 'well', 'cmpdname', 'CONCuM', 'cmpdnum', 'VOLuL']:
        ...
        # If the file is not in PharmBio format, we treat it as Plater format.
        # The Plater format is expected to contain exactly two matrices (Drugs and
        # Concentrations). This structure is validated in scan_csv_plater_matrices().
        try:
